...
parser.add_argument("-s", "--speed", required=False, help="Specify the amount time between successful requests (in seconds). Default is 1 second.")
...
\____/  \__,_||_.__/ \_|   \__,_|/___|/___|\___||_| 
 ------------------------------------ By Ludde ----                     
            ...
            print("\n\n")
            if args.output:
                print(Fore.BLUE + f"Press {Fore.MAGENTA + 'CTRL+C' + Fore.BLUE} to stop the fuzzing and save the results to the file '{Fore.WHITE + args.output + Fore.BLUE}'." + Style.RESET_ALL)
                print("\n")
            else:
                print(Fore.BLUE + f"Press {Fore.MAGENTA + 'CTRL+C' + Fore.BLUE} to stop the fuzzing." + Style.RESET_ALL)
                print("\n")
    if len(good_urls) > 1:
        print(Fore.CYAN + f"Fuzzing finished, found {len(good_urls)} subdomains for {args.domain}!" + Style.RESET_ALL)
        if args.output:
